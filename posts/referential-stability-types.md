...
메모이제이션된 자식 컴포넌트에 새 배열 리터럴을 넘긴 탓에 일어난 리렌더링이나 이펙트 실행을 추적하느라, 인정하고 싶은 것보다 훨씬 많은 시간을 썼습니다. `Item[]`라는 타입 선언이나 prop 타입은 배열의 형태와 배열에 어떤 값이 들어 있는지는 알려 주지만, 다음 렌더링에서도 같은 배열 참조를 받는지는 알려 주지 않습니다.
