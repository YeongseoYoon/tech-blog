...
곡선이 화면에 그려지는 원리는 드 카스텔조 알고리즘이었습니다. 인접한 점들을 t 비율로 내분하기를 반복하는 것이 핵심입니다.

이제 SVG 파일을 열었을 때 `d="M3.5 2C1.57..."` 같은 게 보여도, "3차 베지어로 곡선을 그리고 있구나" 정도는 읽을 수 있게 되었습니다. 다음 편에서는 이걸 바탕으로 SVG 최적화와 아이콘 시스템을 다뤄보겠습니다.
